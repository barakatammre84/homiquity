 ...
 *
 * Outside production. parsePlaidAssetReport refuses to fabricate under
 * NODE_ENV=production unless ASSET_VENDOR_MODE=simulation (F-037), because its
 * output is persisted to a table with no simulation column and is turned into
 * borrower-facing document requests.
 */
...
export async function parsePlaidAssetReport(assetReportToken: string): Promise<ParsedAssetReport> {
  ...
  // Refuse to fabricate asset data in production (F-037). Same shape as
  // softPullCredit / fetchAvm in server/mcp/vendors.ts: the branch above is the
  // only gate, and it cannot fire without Plaid credentials — the actual state —
  // so this returned invented balances and transactions under any NODE_ENV.
  //
  // Those do not stop at a number. routes/aus.ts writes them to
  // verification_reports (which has no is_simulated column to mark them), and
  // runPreUnderwriting turns them into LOW_RESERVES_WARNING and
  // LARGE_DEPOSIT_SOURCING — a borrower is asked for a gift letter sourcing a
  // deposit that was generated by seeded(token + "depamt"), and financialReview
  // raises an asset blocker that holds the file. Measured over 200 tokens in
  // #825: 75 produced a sourcing request, 38 a reserves warning.
  //
  // Throwing here is deliberately before the insert: the webhook's outer catch
  // logs and returns 500, so no unmarked row is persisted and nothing reaches
  // the borrower. Real Plaid credentials take the branch above and never see
  // this guard.
  //
  // Its own variable, not CREDIT_VENDOR_MODE: allowing fabricated credit in a
  // staging environment must not silently allow fabricated assets.
  if (
    process.env.NODE_ENV === "production" &&
    process.env.ASSET_VENDOR_MODE !== "simulation"
  ) {
    throw new Error(
      "Simulated asset reports are disabled in production. Set ASSET_VENDOR_MODE=simulation to explicitly allow fabricated balances and transactions in non-live environments.",
    );
  }
