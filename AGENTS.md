...
Verify the remote is `barakatammre84/homiquity`; the former `homiquity-mortgage-broker` name
still redirects to it, so an older checkout's remote is the same repository, not the wrong one.
Use `origin/main` explicitly; a cached `origin/HEAD` is not proof of GitHub's default branch.
