...
| F-16 | guards in the CI gate | `grep -oE "pnpm guard:[a-z]+" .github/workflows/ci.yml \| sort -u` | authority bundle channel citations conformance corpus coverage gating migrations querykeys schema security sources staleness tokens ui vocab (17). Was 18 with `kb` and `seats`: #811 retired both guards and left them delegating to the one `guard:sources` runs, so the gate ran it twice under three names; collapsed to a single step 2026-09-15 |
| F-17 | pre-push steps · preflight steps · checkup checks | `grep -c '^step ' .githooks/pre-push ; grep -c 'step "' scripts/preflight.sh ; grep -c '^check "' scripts/checkup.sh` | 16 · 28 · 24 (was 10 — `e49aab6d`/#660 made the unit lanes opt-in behind `PREPUSH_TESTS=1`; +the Selling Guide corpus/coverage/watch lines, 2026-08-23. Pre-push 17→16, preflight 29→28, checkup 26→24 on 2026-09-15: the KB-index, doc-freshness and seat-roster lines were all the same script after #811, so each harness reported one guard as two or three passes) |
